...
function InitializeBuildTool() {
  ...
  if ($msbuildEngine -eq "dotnet") {
    ...
    $buildTool = @{ Path = Join-Path $dotnetRoot "dotnet.exe"; Command = "msbuild"; Tool = "dotnet"; Framework = "netcoreapp2.2" }
...
function InitializeToolset() {
  ...
  $toolsetVersion = $GlobalJson.'msbuild-sdks'.'Thunder.BuildSystem.Sdk'
  ...
  '<Project Sdk="Thunder.BuildSystem.Sdk"/>' | Set-Content $proj
...
function MSBuild() {
  if ($pipelinesLog) {
    ...
    $path = Join-Path $path (Join-Path $buildTool.Framework "Thunder.BuildSystem.Sdk.dll")
    $args += "/logger:$path"
  }
