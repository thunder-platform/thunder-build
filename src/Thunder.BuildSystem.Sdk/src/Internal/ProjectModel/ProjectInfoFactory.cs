...
namespace Thunder.BuildSystem.Sdk.Internal.ProjectModel
{
    internal class ProjectInfoFactory
    {
        ...
        public ProjectInfo Create(string path, bool policyDesignBuild)
        {
            ...
            return new ProjectInfo(path, projExtPath, frameworks);
        }
        ...
        private IReadOnlyDictionary<string, PackageReferenceInfo> GetDependencies(ProjectInstance project)
        {
            ...
            foreach (var item in project.GetItems("PackageReference"))
            {
                ...
                {
                    _logger.LogThunderBuildWarning(project.ProjectFileLocation.File, ThunderBuildErrors.DuplicatePackageReference, $"Found a duplicate PackageReference for {info.Id}. Restore results may be unpredictable.");
                }
